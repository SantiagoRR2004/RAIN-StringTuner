...
def createController() -> ctrl.ControlSystemSimulation:
    ...
    frequencyDifference["very_close"] = fuzz.trimf(frequencyDifference.universe, [0, 0, 50])
    frequencyDifference["close"] = fuzz.trimf(frequencyDifference.universe, [0, 150, 300])
    frequencyDifference["medium"] = fuzz.trimf(frequencyDifference.universe, [150, 300, 600])
    frequencyDifference["far"] = fuzz.trimf(frequencyDifference.universe, [500, 1000, 19800])
    ...
    rule1 = ctrl.Rule(
        frequencyDifference["very_close"] & stringLength["small"], turn["very_little"])
    rule2 = ctrl.Rule(
        frequencyDifference["close"] & stringLength["small"], turn["very_little"])
    rule3 = ctrl.Rule(
        frequencyDifference["far"] & stringLength["small"], turn["medium"])
    rule4 = ctrl.Rule(
        frequencyDifference["very_close"] & stringLength["medium"], turn["very_little"])
    rule5 = ctrl.Rule(
    ...
    turn_ctrl = ctrl.ControlSystem([rule1, rule2, rule3, rule4, rule5, rule6, rule7, rule8, rule9])
